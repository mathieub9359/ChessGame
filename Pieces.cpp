/**
La classe `Piece` est une classe abstrait qui possède des enfants qui représente les différentes pièces aux échec. Elle permet de valider si un mouvement est valide de chaqu'une des pièecs ainsi
que de les bouger.
...
bool Piece::canMoveInDiagonale(std::shared_ptr<Piece> chessBoard[8][8], const int toVerifyXPos, const int toVerifyYPos) const {
