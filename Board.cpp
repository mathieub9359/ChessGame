/**
La classe `BoardController` permet de garder la position des pieces, les différentes piece blanches et noirs. Elle permet également de faire divers
vérification tel que les mises en échec, les mouvements illégaux et la création et destructions des différentes pieces.
...
bool Model::BoardController::isAQueen(const int xPos, const int yPos) {
    return dynamic_cast<Queen*>(pieceSquares_[xPos][yPos].get());
}
