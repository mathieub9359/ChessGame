/**
Ce ficher permet de faire divers test pour assurer le bon fonctionnement de l'échiquier et des différentes pièce qui le compose.à
Les tests vérifient les movement valides de chaqu'une des pièces et les différentes mise en échec.

ficher      TestChess.cpp
...
TEST(pawn, promotion) {
	using namespace Model;
	BoardController::createPawnPiece(4, 1, true);

	BoardController::movePieceTo(4, 1, 4, 0);

	EXPECT_TRUE(BoardController::isAQueen(4, 0)) << "The pawn hasn't been promoted to a Queen.";

	BoardController::removePiece(4, 0); //Remove the pawn
